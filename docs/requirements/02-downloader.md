...
## 2.0 当前代码实现状态（2026-07-10）

- 当前已实现：`/api/downloads/check`、`/api/downloads`、任务列表、详情、删除（取消/删除双行为）、重试、SSE 进度流。
- 当前为**单视频流程**：`ScraperService` 与下载 API 都未启用歌单拆解入队。
- 下载格式为严格双选：`video_format_id + audio_format_id` 必填并在后端二次校验。
- 重试机制已实现（最大 3 次，带退避）；后台清理任务循环已挂载但仍是占位逻辑。
