...
| 异步运行时 | **asyncio + create_task + Semaphore(2)** | 见 [07-backend.md](07-backend.md) |
...
## 0.5 当前代码实现状态（2026-07-11）

- **下载主链路**：check/create/list/retry/delete/SSE 全部落地，单视频双 format_id 严格模式。
- **视频模块**：`GET /api/videos`、`GET /api/videos/{id}`、`DELETE`、`batch-delete`、`/thumbnail`、`/stream`（Range 分片）、`PATCH /progress` 全部实现。
- **历史模块**：`GET /api/history`、`DELETE /api/history/{id}`、`POST /api/history/clear` 全部实现。
- **播放链路**：video.js + Range 流式播放 + 进度记忆（`PlayHistory` upsert）已端到端打通。
- **定时清理**：`task_cleaner_loop`（Ready 3天 / Failed&Cancelled 30天）和 `history_cleaner_loop`（30天）均已实现。
- **容器部署**：CWD=`/app`，数据挂载至 HDD `/home/tubehub/data`，代理 `10.158.100.9:8080`，已安装 `deno`。
- **DB schema**：已迁移，移除旧字段 `format_type`/`quality`，路径解析改用 `os.path.abspath()`。
