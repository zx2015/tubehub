...
## 0.5 当前代码实现状态（2026-07-10）

- 下载主链路（check/create/list/retry/delete/SSE）已落地，且为单视频双 format_id 模式。
- 视频模块目前仅 `GET /api/videos` 与 `DELETE /api/videos/{id}` 为真实实现，其余接口仍为占位。
- 历史模块目前仅 `GET /api/history` 为真实实现，删除与清空仍为占位。
- 清理后台循环已启动，但 `task_cleaner_loop`/`history_cleaner_loop` 目前为占位逻辑。
